...
首先，DevOps不是一个具体的工具。你没办法购买它，因为它并不是一个明码标价的软件产品或者Github上的一个开源代码仓库。它也不是某种编程语言，更不是什么暗黑魔法。
...
DevOps是一组实操法则，帮助我们实现：减少一个产品从概念阶段到发布给终端用户使用，这当中的时间。
...
在第一周中，我们需要深入研究的另一个领域是**敏捷方法论** DevOps和敏捷在实现应用的持续交付过程中是广泛适配的。
